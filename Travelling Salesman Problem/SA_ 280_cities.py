...
def main():
    ...
    plt.savefig('SA_c_6000.png', dpi=800) 
    plt.clf() 
    plt.plot( V_itera, V_cost, label='Simulated Annealing Method_ homogenous Markov chain')
    plt.plot( V_itera, V_theory, label='Optimal tour')
    plt.legend(fontsize=9)
    ...
    plt.xlim(0,10000)
    plt.savefig('SA_c_6000_short.png', dpi=800) 
    np.savetxt('280_G.txt',tour)
    np.savetxt('280_G_cost.txt',[cost])
    np.savetxt('280_G_accept.txt',V_accept)
