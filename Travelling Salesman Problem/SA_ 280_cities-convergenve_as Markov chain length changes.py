...
def main():
    'Get data'
    data = np.genfromtxt("a280.tsp.txt",skip_header=6, skip_footer=1)
    data_position = np.genfromtxt("a280.tsp.txt",skip_header=6, skip_footer=1,usecols = (1, 2))
    N_City=len(data)
    ...
    'Simulated Annealing Method_ homogenous Markov chain, under various Markov chain length'
    ...
    V_rep=[]
    #count=0
    for l in range(100,1001,100):# Simulate with various Markov Chain’s length    
        c=100#initial temperature
        ...
        V_cost.append(cost)
        V_theory.append(2586)
        V_stop_temp.append(c)
        V_rep.append(rep)# to compare how quickly it convergrnve to an local minimum.
    plt.plot( range(100,1001,100), V_theory, label='the cost of Optimal tour')
    plt.plot(range(100,1001,100), V_cost,'bo',range(100,1001,100), V_cost,'k')
    plt.xlabel('Markov Chain length')
    plt.legend(fontsize=9)
    plt.savefig('C280_multi_l.png', dpi=400) 
    
      
    plt.clf() 
    plt.plot(range(100,1001,100), V_rep, 'bo',range(100,1001,100), V_rep,'k')
    plt.title('Number of Markov chains in each simulation')
    plt.xlabel('Markov Chain length')
    plt.legend(fontsize=9)
    plt.savefig('C280_multi_l_n.png', dpi=400)     
    np.savetxt('280_multi_l.txt',tour)
    np.savetxt('280_multi_l_cost.txt',[cost])  
