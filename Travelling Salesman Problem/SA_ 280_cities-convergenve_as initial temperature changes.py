...
def main():
    ...
    'Simulated Annealing Method_ homogenous Markov chain, under various initial temperature'
    ...
    plt.plot( range(1,29,4), V_theory, label='the cost of Optimal tour')
    plt.plot(range(1,29,4), V_cost,'bo',range(1,29,4), V_cost,'k')
    plt.xlabel('Initial temperature')
    plt.ylabel('Cost')
    plt.legend(fontsize=9)
    plt.savefig('C280_multi_c.png', dpi=400) 
        
    plt.clf() 
    plt.plot(range(1,29,4), V_rep, 'bo',range(1,29,4), V_rep,'k')
    plt.title('Number of Markov chains in each simulation')
    plt.xlabel('Initial temperature')
    plt.ylabel('Cost')
    plt.legend(fontsize=9)
    plt.savefig('C280_multi_c_n.png', dpi=400)     
    np.savetxt('280_multi_c.txt',tour)
    np.savetxt('280_multi_c_cost.txt',[cost])
