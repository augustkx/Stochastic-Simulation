...
def main():
    ...
    'Simulated Annealing Method_ homogenous Markov chain, under various initial cooling rate'
      
    V_theory=[]
    V_cost=[] 
    V_rate=[]
    V_rep=[]
    for Rate in range(960,1000,4):
        rate=float(Rate)/1000#decrease parameter
        V_rate.append(rate)
        print Rate,rate, V_rate
        c=100#control value/temperature
        tour=initial_tour
        cost=initial_cost
    ...
    print  V_rate
    plt.plot( V_rate, V_theory, label='the cost of Optimal tour')
    plt.plot(V_rate, V_cost,'bo',V_rate, V_cost,'k')
    plt.xlabel('Cooling Rate')
    plt.legend(fontsize=9)
    plt.savefig('C280_multi_r.png', dpi=400) 
        
    plt.clf() 
    plt.plot(V_rate, V_rep,'bo',V_rate, V_rep,'k')
    plt.title('Number of Markov chains in each simulation')
    plt.xlabel('Cooling Rate')
    plt.legend(fontsize=9)
    plt.savefig('C280_multi_r_n.png', dpi=400)     
    np.savetxt('280_multi_r.txt',tour)
    np.savetxt('280_multi_r_cost.txt',[cost])
