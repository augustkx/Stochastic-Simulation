...
def main():
    ...
    for ite in range(100,5050,50):
        ...
        array_ite.append(ite)
        array_n.append(int(n))
        array_mean_area.append(mean_area)
        change.append(array_mean_area[m]-array_mean_area[m-1])
        m += 1
    plt.plot(array_ite, array_mean_area)
    plt.ylim(1.45,1.57)
    plt.xlabel('Number of iterations')
    plt.ylabel('Area of Mandelbrot set')
    plt.title('Pure random sampling \n Convergence of A_i,s as i increases')
    print array_mean_area
    print time.clock()
    np.savetxt('Pure random Sampling_i.txt', array_n)
    np.savetxt('Pure random Sampling_change_mean.txt',change)
    np.savetxt('Pure random Sampling_mean.txt',array_mean_area)
